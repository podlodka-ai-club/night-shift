...
Usage:
  npm run eval:implement -- --fixtures <dir> [options]
...
export function parseEvalImplementCliArgs(argv: ReadonlyArray<string>): CliOptions {
  ...
  if (values.mode === 'live') {
    if (values.record === true && values.judge === true) {
      throw new EvalImplementCliUsageError(64, '--record cannot be combined with --judge\n', 'stderr');
    }
  ...
  if (values.record === true) {
    throw new EvalImplementCliUsageError(64, '--record requires --mode live\n', 'stderr');
  }
  ...
  if (liveOnlyFlags.length > 0) {
    throw new EvalImplementCliUsageError(64, `${liveOnlyFlags.join(', ')} ${liveOnlyFlags.length === 1 ? 'is' : 'are'} only supported in live mode\n`, 'stderr');
  }
    ...
    const requiredFlag =
      maxRevisionsValue !== undefined ? '--max-revisions'
        : provider !== undefined ? '--judge-provider'
          : '--judge-model';
    throw new EvalImplementCliUsageError(64, `${requiredFlag} requires --judge\n`, 'stderr');
  }
...
function isSuccessfulRecordingResult(result: ImplementReplayResult): boolean {
...
function isDirectCliExecution(argv: ReadonlyArray<string> = process.argv): boolean {
  ...
  return [
    path.join('src', 'cli', 'eval-implement.ts'),
    path.join('lib', 'cli', 'eval-implement.js'),
