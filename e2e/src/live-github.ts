...
function assertCommonWorkflowArtifacts(
  ...
  assert.ok(
    comments.some((comment) => comment.body === buildIssueComment(workflowResult.pullRequestUrl)),
    'Expected the seeded issue to contain the pull request comment.',
  );
}
...
async function assertFakeAgentArtifacts(
  ...
  assert.deepStrictEqual(actualArtifacts, getExpectedFakeArtifacts(seededIssue.runId));
}
...
function getExpectedFakeArtifacts(runId: string): FakeArtifactsSnapshot {
  return {
    commitMessage: `test: fake e2e change for ${runId}`,
    pullRequestTitle: `test: fake e2e PR for ${runId}`,
    pullRequestBody: `## Summary\n- create the deterministic fake e2e change\n- run marker: ${runId}`,
    fileText: ['# Fake E2E Change', '', `Run marker: ${runId}`].join('\n'),
  };
}
