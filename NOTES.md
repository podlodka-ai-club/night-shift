...
- Status: complete
- Notes:
  - [Origin: Task 9 | Relevant to: future maintenance] The final steady-state cleanup policy is now explicit: successful phased runs call `cleanupWorktree` after returning `Ready to merge`, while blocked/failing/escalated paths preserve the local worktree for debugging and resume. Cleanup failures are best-effort and do not replace a successful workflow result.
  - [Origin: Task 9 | Relevant to: future maintenance] `createWorktreeForIssueIfNeeded` now validates existing worktrees canonically (`realpath`-normalized `rev-parse --show-toplevel`) and recovers from both corrupt-directory and stale-git-admin cases. Recovery handles: broken worktree directories, missing branch metadata, stale `used by worktree` registrations via `git worktree prune`, and the directory-already-gone case by retrying worktree creation after tolerant cleanup.
  - [Origin: Task 9 | Relevant to: future maintenance] The push policy remains intentionally non-forced for automation-owned per-ticket branches. `commitAndPush` is now retry-safe in the important post-push replay case: if a retry sees `origin/<branch>` already at `HEAD`, it exits successfully instead of throwing a false “no changes to push” error.
  - [Origin: Task 9 | Relevant to: future maintenance] The shared intake layer (`intake.ts`) and generic `runAgentSequence` activity remain intentional steady-state seams rather than migration leftovers. The README now documents that architecture directly, including deterministic workflow ids (`ticket-<issueNumber>`), pickup/manual intake, and cleanup/preservation semantics.
  - [Origin: Task 9 | Relevant to: future maintenance] Repo verification flow is hardened: `Makefile` now builds `orchestrator` before `e2e` tests consume compiled `orchestrator/lib` APIs, and workflow test hooks now use explicit 30s setup/teardown timeouts with safe teardown guards to avoid false failures from slow Temporal test-environment startup.
  - [Origin: Task 9 | Relevant to: future maintenance] Task 9 verification is green via focused worktree/workflow regression suites, final `make check`, multiple no-blocker review passes, and a live fake-agent E2E run on 2026-04-28 against `Mugenor/orchestrator-testing` + Project `Mugenor/1` (`runId=8280196a`, issue `#54`, PR `#55`, workflow `ticket-54`, statuses `Ready -> In progress -> In review -> In progress -> In review -> Ready to merge`, cleanup succeeded). GitHub self-review 422s were tolerated by the existing fallback behavior and did not block completion.

### Task 10

- Status: complete
- Notes:
  - [Origin: Task 10 | Relevant to: future maintenance] Donor-compatible config loading now lives in `orchestrator/src/config.ts` and `orchestrator/src/entrypoint-config.ts`. Keep `orchestrator.config.ts` as the canonical filename, continue accepting donor alias/env inputs (`night-shift.config.ts`, `NIGHT_SHIFT_CONFIG`), and preserve the current precedence contract: `--config` > `ORCHESTRATOR_CONFIG`/`NIGHT_SHIFT_CONFIG` > discovered config file > env/CLI fallbacks > built-in defaults.
  - [Origin: Task 10 | Relevant to: future maintenance] `client.ts` and `worker.ts` now share resolved config loading, but `e2e/src/config.ts` remains the intentional temporary exception called out in the task doc/README. Future config work should either migrate that harness explicitly or keep the exception documented instead of partially routing live E2E through the entrypoint loader.
  - [Origin: Task 10 | Relevant to: future maintenance] Live fake-agent verification is green on 2026-04-29 against `Mugenor/orchestrator-testing` + Project `Mugenor/1` when launched with `GITHUB_TOKEN="$(gh auth token)" E2E_TARGET_REPO=Mugenor/orchestrator-testing E2E_PROJECT_OWNER=Mugenor E2E_PROJECT_NUMBER=1 make e2e-live-fake` (`runId=c5c2d360`, issue `#58`, PR `#59`, workflow `ticket-58`, statuses `Ready -> In progress -> In review -> Ready -> In progress -> In review -> Ready to merge`, cleanup succeeded).
  - [Origin: Task 10 | Relevant to: future maintenance] PR self-review GitHub 422s are now converted in `createPullRequestReviewActivity` into non-retryable `GitHubSelfReviewNotAllowed` failures so the review phase can degrade cleanly to `COMMENT` without repeated Temporal retries. Expect at most one warning per blocked self-review attempt unless/until the workflow grows pre-detection for distinct author/reviewer identities.

### Task 11

- Status: complete
- Notes:
  - [Origin: Task 11 | Relevant to: future maintenance] Donor-style scheduled pickup now boots from `orchestrator/src/worker.ts` via the stable Temporal schedule id `pickup-schedule`. The worker creates-or-updates the schedule idempotently, uses `ScheduleOverlapPolicy.SKIP`, and triggers an immediate tick on first create so startup does not wait for the first interval.
  - [Origin: Task 11 | Relevant to: future maintenance] Scheduled pickup remains a thin orchestration layer: `pickupWorkflow` in `orchestrator/src/workflows.ts` delegates to pickup activities, while the actual board scan + start/signal semantics still reuse `orchestrator/src/intake.ts` through `orchestrator/src/pickup.ts` / `orchestrator/src/pickup-activities.ts`. Future webhook work should route through the same intake seam instead of adding a parallel trigger table.
  - [Origin: Task 11 | Relevant to: future maintenance] Pickup configuration now lives in the shared config loader under `pickup.enabled`, `pickup.intervalSeconds`, and `pickup.maxConcurrent`, and this repo intentionally differs from the donor by enabling pickup by default. Operators can opt out explicitly with `pickup.enabled = false`, but the default worker path now expects GitHub project coordinates so scheduled pickup can start workflows autonomously.
  - [Origin: Task 11 | Relevant to: future maintenance] The live fake-agent harness now supports `E2E_INTAKE_MODE=pickup` (`make e2e-live-fake-pickup`) to exercise `pickupWorkflow` directly instead of the manual intake path. Verified green on 2026-04-29 against `Mugenor/orchestrator-testing` + Project `Mugenor/1` with run `4ae63bb4`, issue `#60`, PR `#61`, workflow `ticket-60`, observed statuses `Ready -> In progress -> In review -> In progress -> In review -> Ready to merge`, and cleanup succeeded.
