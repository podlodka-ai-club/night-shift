...
import {
  createImplementCritiqueReviseRunner,
  createImplementLiveRunner,
  type ClosableImplementTurnRunner,
} from "./eval-implement-live.js";
import {
  createImplementJudge,
  type ImplementJudgeProvider,
  type ImplementJudgeVerdict,
} from "./eval-implement-judge.js";

const USAGE = `night-shift eval:implement

Usage:
  night-shift eval:implement --fixtures <dir> [options]
...
Options:
  ...
  --max-revisions <n> Max implementer revision turns triggered by the judge
...
interface CliOptions {
  ...
  record: boolean;
  judge: boolean;
  judgeProvider: ImplementJudgeProvider;
  judgeModel: string;
  maxRevisions: number;
}
...
function parseCliArgs(argv: ReadonlyArray<string>): CliOptions {
  ...
  const judgeProvider = judgeProviderRaw as ImplementJudgeProvider;
...
async function loadFixtures(dir: string): Promise<LoadedFixture[]> {
  ...
  for (const entry of entries) {
    ...
    out.push({
      fixture: ImplementEvalFixtureSchema.parse(JSON.parse(raw)),
      filePath,
    });
  }
...
function makeRecordingRunner(
  inner: ImplementTurnRunner,
): {
  runner: ImplementTurnRunner;
  captured: Map<string, Awaited<ReturnType<ImplementTurnRunner["run"]>>>;
} {
  const captured = new Map<string, Awaited<ReturnType<ImplementTurnRunner["run"]>>>();
  const runner: ImplementTurnRunner = {
    async run(fixture) {
...
function renderText(
  ...
  summary: ImplementEvalSummary,
  mode: "replay" | "live",
): string {
  const lines: string[] = [];
  lines.push(`Implement eval — ${mode} mode`);
  lines.push("─".repeat(40));
  for (const r of results) {
    const flag = r.expectationMismatch ? "FAIL" : "ok  ";
    const cost = `$${(r.costMicroUsd / 1_000_000).toFixed(4)}`;
    lines.push(
      `  ${flag}  ${r.id.padEnd(28)} status=${r.status.padEnd(13)} files=${r.filesWrittenCount} cost=${cost}`,
    );
    if (r.expectationMismatch) lines.push(`        mismatch: ${r.expectationMismatch}`);
  }
  lines.push("─".repeat(40));
  lines.push(`  total:                ${summary.total}`);
  lines.push(`  produced:             ${summary.byStatus.produced}`);
  lines.push(`  empty:                ${summary.byStatus.empty}`);
...
function renderJudgeLog(
  judgeLog: ReadonlyArray<{ fixtureId: string; attempt: number; verdict: ImplementJudgeVerdict }>,
): string {
...
export async function main(argv: ReadonlyArray<string>): Promise<number> {
  ...
  let liveRunner: ClosableImplementTurnRunner | undefined;
  let baseRunner: ImplementTurnRunner;
  const judgeLog: Array<{ fixtureId: string; attempt: number; verdict: ImplementJudgeVerdict }> = [];
  if (opts.mode === "live") {
    if (opts.judge) {
      const judge = createImplementJudge({ provider: opts.judgeProvider, model: opts.judgeModel });
      liveRunner = createImplementCritiqueReviseRunner({
      ...
      liveRunner = createImplementLiveRunner({ provider: opts.provider, model: opts.model });
    }
    baseRunner = liveRunner;
  } else {
    baseRunner = implementReplayRunner;
  }
  ...
  try {
    ...
    const { results, summary } = await evaluateImplementSuite(fixtures, runner);
